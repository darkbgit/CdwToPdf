...
using CdwHelper.Core.Analyzers.Ver22.XmlModel;
using CdwHelper.Core.Enums;
using CdwHelper.Core.Interfaces;
using CdwHelper.Core.Models;
using HtmlAgilityPack;

namespace CdwHelper.Core.Analyzers.Ver22;

internal partial class RootPartAnalyzer22 : IRootPartAnalyzer
{
    ...
    private static Root22 Deserialize(Stream xml)
    {
        try
        {
            XmlSerializer formatter = new(typeof(Root22));
            return formatter.Deserialize(xml) as Root22
    ...
    private static KompasDocument? AnalyzeSpecification(Root22 xmlDoc)
    {
    ...
    private static KompasDocument? Analyze2D(Root22 xmlDoc)
    {
        var docSection = xmlDoc.Product.Documents
    ...
    private static KompasDocument? Analyze3D(Root22 xmlDoc)
    {
        var docSection = xmlDoc.Product.Documents
